...
module Nokul
  module Support
    class CodingPrefixedGeneratorTest < ActiveSupport::TestCase
      test 'basic use case should just work' do
        generator = Coding::PrefixedGenerator.new '123', prefix: %w[203 19]
        number = generator.generate
        assert_equal Coding::PrefixedGenerator.length, number.length
      ...
      test 'giving a nil starting sequence should reset generator' do
        generator = Coding::PrefixedGenerator.new nil, prefix: %w[203 19]
      ...
      test 'giving a blank starting sequence should also reset generator' do
        generator = Coding::PrefixedGenerator.new '', prefix: %w[203 19]
        assert_equal '20319001', generator.generate
        assert_equal '20319002', generator.generate
        assert_equal '003', generator.next_sequence
      end
      ...
      test 'lack of a trailing prefix should produce long sequences' do
        generator = Coding::PrefixedGenerator.new '00123', prefix: '203'
      ...
      test 'can pass extra options to the decorated generator' do
        generator = Coding::PrefixedGenerator.new nil, prefix: '203', deny: /0/
        assert_equal '00001', generator.initial_sequence
      ...
      test 'too long starting sequences should raise exception' do
        exception = assert_raise(Coding::PrefixedGenerator::Error) do
          Coding::PrefixedGenerator.new '1234567', prefix: %w[203 19]
        end
      ...
      test 'too short starting sequences should raise exception' do
        exception = assert_raise(Coding::PrefixedGenerator::Error) do
          Coding::PrefixedGenerator.new '123', prefix: '203'
        end
        assert_equal 'Incorrect length for starting sequence: 123', exception.message
      end

      class TooShortGenerator < Coding::PrefixedGenerator
        self.length = 2
      end
      ...
      test 'too long prefixes should raise exception' do
        exception = assert_raise(Coding::PrefixedGenerator::Error) do
          Coding::PrefixedGenerator.new '123', prefix: %w[12345 19]
        end
