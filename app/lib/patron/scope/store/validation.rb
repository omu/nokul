          ...
          def set_accessors_validations
            [*scope_klass&.filter_attributes].each do |filter|
              unless skip_empty_for?(filter)
                prefix = public_send("#{filter}_value_type") == 'dynamic' ? '_dynamic' : ''

                validates_presence_of "#{filter}#{prefix}_value".to_sym
                validates_presence_of "#{filter}#{prefix}_query_type".to_sym
              end
